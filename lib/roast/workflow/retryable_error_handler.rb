...
module Roast
  module Workflow
    ...
    class RetryableErrorHandler < ErrorHandler
      def with_error_handling(step_name, resource_type: nil, retry_policy: nil)
        if retry_policy
          execute_with_retry(step_name, resource_type, retry_policy)
        else
          super(step_name, resource_type: resource_type) { yield }
        end
      end

      private

      def execute_with_retry(step_name, resource_type, retry_policy)

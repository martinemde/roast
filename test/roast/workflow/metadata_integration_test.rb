...
module Roast
  module Workflow
    class MetadataIntegrationTest < ActiveSupport::TestCase
      ...
      test "metadata accessible through workflow" do
        workflow = BaseWorkflow.new

        # Store metadata using hash interface
        workflow.metadata["process_data"] = {
        ...
        assert_includes workflow.metadata["process_data"].keys, "start_time"
        assert_includes workflow.metadata["process_data"].keys, "input_size"
      end
    end
  end
end
